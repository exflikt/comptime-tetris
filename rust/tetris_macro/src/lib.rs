...
pub fn run(input: TokenStream) -> TokenStream {
    if !input.is_empty() {
        panic!("`comptime_tetris` does not accept any arguments");
    }

    let res = run_tetris();
    println!("{:?}", res);

    //proc_macro::TokenTree::Literal(proc_macro::Literal::string(&res)).into()
    proc_macro::TokenTree::Literal(proc_macro::Literal::u32_suffixed(res.unwrap_or(0))).into()
}
